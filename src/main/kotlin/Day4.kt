fun main(args: Array<String>) {
    ...
    input.lines().map {
        val sacks = it.toCharArray().toList().chunked(it.length / 2)
        sacks[0]
            .intersect(sacks[1].toSet())
            .sumOf {
                val number = if (it in 'A'..'Z') {
                    it - 'A' + 27
                } else {
                    it - 'a' + 1
                }
    ...
    input.lines().windowed(3, 3).map {

        it[0].toCharArray().toList().intersect(
            it[1].toCharArray().toList().toSet()
        ).intersect(
            it[2].toCharArray().toList().toSet()
        ).sumOf {
            val number = if (it in 'A'..'Z') {
                it - 'A' + 27
            } else {
