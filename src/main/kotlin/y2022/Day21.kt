...
class Day21(private val input: String) {
    ...
    data class Monkey(val name: String, val first: String, val second: String, val operand: String)
    data class MonkeyResolved(val name: String, val first: String, val second: String, val operand: String)
    data class MonkeyVal(val name: String, val value: Int)
    fun part1(): Long {
        val lines = input.lines().map {
            it.replace(":","").split(" ")
        }
        
        val a = ArrayDeque<Monkey>()
        ...
        fun resolve (monkey: List<String>): Long {
            if (monkey.size == 2) {
                ...
                val monkey1 = lines.first { it.first() == monkey[1] }
                val monkey1Resolved = resolve(monkey1)
                val monkey2 = lines.first { it.first() == monkey[3] }
                val monkey2Resolved = resolve(monkey2)
                return when {
                    monkey[2] == "+" -> { monkey1Resolved + monkey2Resolved }
                    monkey[2] == "-" -> { monkey1Resolved - monkey2Resolved }
                    monkey[2] == "*" -> { monkey1Resolved * monkey2Resolved }
                    monkey[2] == "/" -> { monkey1Resolved / monkey2Resolved }
                    else -> error("idk")
    ...
    fun part2(): Long {
        ...
        fun resolve (monkey: List<String>): Long {
            if (monkey.size == 2) {
                ...
                val monkey1 = lines.first { it.first() == monkey[1] }
                val monkey1Resolved = if (monkey1.first() == "humn") {
//                    println("human using $human")
                    human
                } else {
                    resolve(monkey1)
                }
