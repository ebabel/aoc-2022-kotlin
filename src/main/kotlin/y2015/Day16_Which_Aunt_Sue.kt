...
private const val SUE_REGEX = """Sue (\d+): (\w+): (\d+), (\w+): (\d+), (\w+): (\d+)"""
private const val MFCSAM_REGEX = """(\w+): (\d+)"""
fun main(args: Array<String>) {

//    println("Part1 test ${part1(testInput)}")
    println("Part1 real ${part1()}")
//    println("Part2 test ${part2(testInput)}")
    println("Part2 real ${part2()}")
}

private data class Sue(val index: String, val attr: List<String>)
...
private fun part1(): String {
    ...
    val allAunts =
        input1
            .lines()
            .map { SUE_REGEX.toRegex().matchEntire(it)!!.destructured.toList() }
//            .also(::println)
            .map {
                Sue(it[0],it.drop(1).chunked(2){ "${ it.first() }: ${ it.last() }"})
            }
//            .also{ it.forEach{println(it)} }
    val mfcsam = input2.lines().flatMap { MFCSAM_REGEX.toRegex().matchEntire(it)!!.destructured.toList() }

    val mfcsam1 = mfcsam.chunked(2).map { "${it.first()}: ${it.last()}" }.also(::println)
    val allAunts1 = allAunts.chunked(2).map { "${it.first()}: ${it.last()}" }

    var allAunts2 = allAunts
//    mfcsam1.forEach {mfc ->
//        println(mfc)
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("cats") }) {
                sue.attr.any { it.contains("cats") && it.split(" ").last().toInt() > 7 }
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("trees") }) {
                sue.attr.any { it.contains("trees") && it.split(" ").last().toInt() > 3 }
            } else {
                true
            }
        }
        ...
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("pomeranians") }) {
                sue.attr.any { it.contains("pomeranians") && it.split(" ").last().toInt() < 3 }
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("goldfish") }) {
                sue.attr.any { it.contains("goldfish") && it.split(" ").last().toInt() < 5 }
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("children") }) {
                sue.attr.any { it == "children: 3"}
            } else {
                true
            }
        }
        ...
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("samoyeds") }) {
                sue.attr.any { it == "samoyeds: 2"}
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("akitas") }) {
                sue.attr.any { it == "akitas: 0"}
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("vizslas") }) {
                sue.attr.any { it == "vizslas: 0"}
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("cars") }) {
                sue.attr.any { it == "cars: 2"}
            } else {
                true
            }
        }
        allAunts2 = allAunts2.filter { sue ->
            if (sue.attr.any { it.contains("perfumes") }) {
                sue.attr.any { it == "perfumes: 1"}
            } else {
                true
            }
        }
...
//    }
    ...
    println(allAunts2.size)
//    allAunts.filter {
////        it.attr.any { it == "children: 3" }
//    }.filter {
//        it.attr.any { it == "cats: 7" }
//    }.forEach {
//        println(it)
//    }
