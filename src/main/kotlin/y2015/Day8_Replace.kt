...
private fun part1(testInput1: String) {
    testInput1.lines().map { line ->
        line
            .dropLast(1)
            .drop(1)
            .replace("$BACKSLASH$QUOTE", "-")
            .replace("$BACKSLASH$BACKSLASH", "!")
            .replace("""\\x\w{2}""".toRegex(), "'") //   \x23 -> '
            .also {
//                println("$line ${line.length} $it ${it.length}")
            }.let {
                line.length - (it.length)
            }
    }.sum().also(::println)
}
...
private fun part2(testInput1: String) {
    testInput1.lines().map { line ->
        line
            .replace("$BACKSLASH$QUOTE", "$$$$")
            .replace("$BACKSLASH$BACKSLASH", "!!!!")
            .replace("""\\x\w{2}""".toRegex(), "'".repeat(5)) //   \x23 -> '
            .also {
//                println("$line ${line.length} $it ${it.length+4}")
            }.let {
                 (it.length+4)-line.length
            }
    }.sum().also(::println)
}
