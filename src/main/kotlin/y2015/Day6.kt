...
private fun part1() {
    ...
    input.lines().forEach { lineString ->
        val line = lineString.split(" ")
        val corner1 = if (line[0] == "turn") {
            line[2]
        } else {
            line[1]
        }.split(",").map { it.toInt() }

        val corner2 = line.last().split(",").map { it.toInt() }

        (corner1[1]..corner2[1]).forEach { y ->
            (corner1[0]..corner2[0]).forEach { x ->
                if (line[0] == "toggle") {
...
private fun part2() {
//    val lights = Array(5) { Array(5) { 0 } }
    val lights = Array(1000) { Array(1000) { 0 } }

//    testinput.lines().forEach { lineString ->
    input.lines().forEach { lineString ->
        val line = lineString.split(" ")
        val corner1 = if (line[0] == "turn") {
            line[2]
        ...
        (corner1[1]..corner2[1]).forEach { y ->
            (corner1[0]..corner2[0]).forEach { x ->
                if (line[0] == "toggle") {
                    ...
                    lights[x][y] += -1
                    lights[x][y] = lights[x][y].coerceAtLeast(0)
                }
            }
        }
    }
...
toggle 296,687 through 906,775""".trimIndent()